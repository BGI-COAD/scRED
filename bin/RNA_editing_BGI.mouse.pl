...
`$Bin/MismatchStat -i $rnabam  -x 1000000 -q 20 -o $output/rna_mismatch_stat.txt`;

#Calling varitation sites;
print "Variation sites detecting...\n";
#`$Bin/MutDet -i $rnabam -r $reference  -v $output/rna_mismatch_stat.txt -u -o $output/var.sites.txt.gz`#;
`$Bin/MutDet -i $rnabam -r $reference  -v $output/rna_mismatch_stat.txt -q 20 -o $output/var.sites.txt.gz`;
...
foreach my $dbsnpfile (@list){
    ...
    while (<SNP>){
        ...
        if ($_ =~ /^#/) {next;}
        my @array = split /\s+/,$_;
        print TEMP_SNP "chr".$array[0]."\t".$array[1]."\n";
    }
    close SNP;
}
