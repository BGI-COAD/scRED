...
#run step 1
if ($valistep{1}) {
	$sc = "annotate_variation.pl -geneanno -buildver $buildver -outfile $outfile -exonsort $queryfile $dbloc";
...
if ($buildver eq 'hg19') {
	if ($valistep{4} or $valistep{5} or $valistep{6}) {
		$sc = "annotate_variation.pl -filter -dbtype 1000g2010nov_all -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	#run step4
	if ($valistep{4}) {
		$sc = "annotate_variation.pl -filter -dbtype 1000g2010jul_ceu -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	#run step5
	if ($valistep{5}) {
		$sc = "annotate_variation.pl -filter -dbtype 1000g2010jul_yri -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	#run step6
	if ($valistep{6}) {
		$sc = "annotate_variation.pl -filter -dbtype 1000g2010jul_jptchb -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	if (not @{$allstep[1]}) {
		if ($valistep{1}) {
			if (defined ($_ = <STEP1>)) {
				...
				if (@aachange >= 5) {
					push @{$allstep[1]}, $varstring, $efun, "$aachange[0]:$aachange[1]:$aachange[3]:$aachange[4]";
				} else {
