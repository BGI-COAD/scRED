...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'step=s'=>\$step, 
	'checkfile!'=>\$checkfile, 'remove'=>\$remove, 'verdbsnp=i'=>\$verdbsnp) or pod2usage ();
...
#run step 1
if ($valistep{1}) {
	$sc = "$Bin/annotate_variation.pl -geneanno -buildver $buildver -outfile $outfile -exonsort $queryfile $dbloc";
...
if ($valistep{2}) {
	if ($buildver eq 'hg18') {
		$sc = "$Bin/annotate_variation.pl -regionanno -dbtype mce44way -buildver $buildver -outfile $outfile $queryfile $dbloc";
		print STDERR "\nNOTICE: Running step 2 with system command <$sc>\n";
		system ($sc) and die "Error running system command: <$sc>\n";
	} elsif ($buildver eq 'hg19') {
		$sc = "$Bin/annotate_variation.pl -regionanno -dbtype mce46way -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
#run step3
if ($valistep{3}) {
	$sc = "$Bin/annotate_variation.pl -regionanno -dbtype segdup -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
if ($buildver eq 'hg19') {
	if ($valistep{4} or $valistep{5} or $valistep{6}) {
		$sc = "$Bin/annotate_variation.pl -filter -dbtype 1000g2010nov_all -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	#run step4
	if ($valistep{4}) {
		$sc = "$Bin/annotate_variation.pl -filter -dbtype 1000g2010jul_ceu -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	#run step5
	if ($valistep{5}) {
		$sc = "$Bin/annotate_variation.pl -filter -dbtype 1000g2010jul_yri -buildver $buildver -outfile $outfile $queryfile $dbloc";
	...
	#run step6
	if ($valistep{6}) {
		$sc = "$Bin/annotate_variation.pl -filter -dbtype 1000g2010jul_jptchb -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
#run step7
if ($valistep{7}) {
	$sc = "$Bin/annotate_variation.pl -filter -dbtype snp$verdbsnp -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
#run step8
if ($valistep{8}) {
	$sc = "$Bin/annotate_variation.pl -filter -dbtype avsift -buildver $buildver -sift 0 -outfile $outfile $queryfile $dbloc";
...
#run step9
if ($valistep{9}) {
	$sc = "$Bin/annotate_variation.pl -filter -dbtype ljb_pp2 -score_threshold 0 -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
#run step10
if ($valistep{10}) {
	$sc = "$Bin/annotate_variation.pl -filter -dbtype ljb_phylop -score_threshold 0 -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
#run step11
if ($valistep{11}) {
	$sc = "$Bin/annotate_variation.pl -filter -dbtype ljb_mt -score_threshold 0 -buildver $buildver -outfile $outfile $queryfile $dbloc";
...
#run step12
if ($valistep{12}) {
	$sc = "$Bin/annotate_variation.pl -filter -dbtype ljb_lrt -score_threshold 0 -buildver $buildver -outfile $outfile $queryfile $dbloc";
